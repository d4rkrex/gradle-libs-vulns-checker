...
# Super Vuln Checker v4.5
# Dependency-Check + Resolved Dependencies Listing (OSV-Scanner commented out)
...
echo -e "${CYAN}- Super Vuln Checker v4.5 started...${NC}"

if [ "$#" -lt 1 ]; then
  echo -e "${RED}❌ Invalid usage.${NC}"
  echo "Usage:"
  echo "  $0 group:artifact:version"
  echo "  $0 dependencies.txt (one per line)"
  exit 1
fi
...
if [[ "$MODE" == "file" && ! -f "$INPUT" ]]; then
  echo -e "${RED}❌ File not found or not a regular file: $INPUT${NC}"
  echo "💡 Hint: did you mount your folder using -v $(pwd):/app and ensure $1 exists?"
  exit 1
fi
...
if [[ "$MODE" == "file" ]]; then
  echo -e "📄 Reading dependencies from file: ${YELLOW}$INPUT${NC}"
...
# OSV-Scanner code commented out
...
#   exit 4
# fi
# echo -e "${CYAN}🛡️ Running OSV-Scanner...${NC}"
